...
# filter -999 values NA values
avg_ml<- moana %>% filter(value != -999.99) %>% group_by(year) %>% summarise_at(vars(value), list(mean))
...
core<- dplyr::filter(core, `Year AD` > 1900 & `Year AD` < 1974) # everything above 1974 to combine with ML data
...
#write.csv(c02data, paste0(getwd(), "/Other_environmental_data/c02_data/c02_MOANA_NOAA_combined.csv"), row.names =F)
