...
#setwd("C:/Users/GonzalezB2/Desktop/Smithsonian/CEDS/")
...
args_from_makefile <- base::commandArgs(TRUE) # make sure to add this
em <- args_from_makefile[ 1 ]
if ( is.na( em ) ) em <- "SO2"  # change to nox or s02
...
setwd("C:/Users/GonzalezB2/Desktop/Smithsonian/CEDS/final-emissions/")
if ( GRID_SUBREGIONS ) {
...
gridding_emissions <- ceds_gridding_mapping %>%
  ...
  dplyr::inner_join( emissions, by = c( 'CEDS_working_sector' = 'sector' ) ) %>% #join emissions with country location dataset
  dplyr::filter( !is.na( CEDS_int_gridding_sector_short ) ) %>% # remove NA
  dplyr::group_by( iso, CEDS_int_gridding_sector_short ) %>% # group by country
  dplyr::summarise_at( paste0( 'X', year_list ), sum ) %>% # summarise the sum of years
  dplyr::ungroup() %>% # start a new analysis
