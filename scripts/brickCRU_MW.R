...
#### ### ###
# 24 April 2014
# Amy Bennett, updated in 2015 by Maria Wang and in 2018 by Valentine Herrmann
...
points <- read.csv('R:/Global Maps Data/1. R Code and Instructions/Coordinates Shapefiles/ForestGEOcoordinates.csv')

head(points)
#long and lat
coordinates(points)<-c("modlon", "modlat") #modified coords for sites that fell into ocean/water
...
#make this back to a normal dataframe to check
#as.data.frame(points)


###   --- Set and check the working directory ----------------------------------------------------------- ####
setwd('R:/Global Maps Data/CRU/v4.01/') 
getwd()
list.files()
...
gzfilenames <- paste('gzfiles/', list.files(path = "gzfiles/", pattern = "\\.nc.gz$"), sep='')
...
for(v in c("cld", "dtr", "frs", "pet", "pre", "tmn", "tmp", "tmn", "tmx", "vap", "wet")) {
  ...
  r <- brick(paste0("ncfiles/cru_ts4.01.1901.2016.", v, ".dat.nc"), varname = v)   #maybe ~3mins
  x <- extract(r, points) #maybe ~15 mins #, ncol=2
  head(x)
  x <- cbind(sites.sitename = gsub(" ", "_", points@data$Site), x)
  write.csv(x, file = paste0(v, ".1901.2016-ForestGEO_sites-8-18.csv"), row.names = F)
}
